...
public class ImageListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == TYPE_CAMERA) {
            View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.image_selector_item_camera, parent, false);
            return new HeaderViewHolder(view);
        } else {
            View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.image_selector_item_picture, parent, false);
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        if (getItemViewType(position) == TYPE_CAMERA) {
            ...
            Glide.with(context)
                    ...
                    .centerCrop()
                    .thumbnail(0.5f)
                    .placeholder(R.drawable.image_selector_image_placeholder)
                    .error(R.drawable.image_selector_image_placeholder)
                    .dontAnimate()
    ...
    public void selectImage(ViewHolder holder, boolean isChecked) {
        ...
        if (isChecked) {
            holder.picture.setColorFilter(context.getResources().getColor(R.color.image_selector_image_overlay2), PorterDuff.Mode.SRC_ATOP);
        } else {
            holder.picture.setColorFilter(context.getResources().getColor(R.color.image_selector_image_overlay), PorterDuff.Mode.SRC_ATOP);
        }
    }
